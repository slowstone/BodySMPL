...
def rodrigues(r):
    ...
    theta = tf.maximum(theta, np.finfo(np.float32).tiny)
    r_hat = r / theta
    cos = tf.cos(theta)
...
def with_zeros(x):
    ...
    """
    ret = tf.concat(
        (x, tf.constant([[0.0, 0.0, 0.0, 1.0]], dtype=tf.float32)),
        axis=0
    )
    return ret
...
def pack(x):
    ...
    """
    ret = tf.concat(
        (tf.zeros((x.get_shape().as_list()[0], 4, 3), dtype=tf.float32), x),
        axis=2
    )
    return ret

def smpl_model(model_path, betas, pose, trans, simplify=False, face_return=False):
    """
  ...
  # For detailed comments see smpl_np.py
    ...
    shapedirs = tf.constant(np.array(params['shapedirs']), dtype=np.float32)
    f = params['f']
    ...
    if simplify:
        ...
        R_cube = R_cube_big[1:]
        I_cube = tf.expand_dims(tf.eye(3, dtype=tf.float32), axis=0) + \
                 tf.zeros((R_cube.get_shape()[0], 3, 3), dtype=tf.float32)
    ...
    results = stacked - \
            pack(
              tf.matmul(
                stacked,
                tf.reshape(
                  tf.concat((J, tf.zeros((24, 1), dtype=tf.float32)), axis=1),
                  (24, 4, 1)
                )
              )
            )
    ...
    rest_shape_h = tf.concat(
        (v_posed, tf.ones((v_posed.get_shape().as_list()[0], 1), dtype=tf.float32)),
        axis=1
    )
    v = tf.matmul(T, tf.reshape(rest_shape_h, (-1, 4, 1)))
    v = tf.reshape(v, (-1, 4))[:, :3]
    result = v + tf.reshape(trans, (1, 3))
#     result = tf.cast(result,dtype=tf.float32,name='verts')
#     result = tf.add((v + tf.reshape(trans, (1, 3))),name='verts')
    if face_return:
        return result,f
    else:
        return result
...
if __name__ == '__main__':
    ...
    output, faces = smpl_model('./model.pkl', betas, pose, trans, True, True)
    sess = tf.Session()
